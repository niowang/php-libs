...
class SsoService
{
    ...
    public function getLoginUri()
    {
        ...
        if(!$ssoLoginUrl = $this->userData['sso_login_url']){
            $ssoLoginUrl = SsoHelper::getSsoLoginUrl();
        }
        ...
        $url = $ssoLoginUrl . '?redirect_uri=' . $redirectUrl;
        return $url;
    }
        ...
        if (!$result) {
            Utils::alert('单点登录鉴权异常',
                json_encode([
                    ...
                    'ip' => $ip,
                    'actionId' => $actionId,
                ], 256), ['chengxusheng1114@dingtalk.com', 'zhongfucheng1139@dingtalk.com']);
